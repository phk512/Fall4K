...
bool initSystemDLLs() {
	...
	if (dynTimeBeginPeriod) dynTimeBeginPeriod(1);
...
long long gameStartTime = 0;          // 点击谱面的时刻，音乐延迟播放
...
bool musicStarted = false;            // 标记音乐是否已播放
long long countdownStartTime = 0;
const long long COUNTDOWN_DURATION = 2000; // 2秒

// ================= 工具与文件函数 =================

long long parseTime(const std::string& timeStr) {
	int min, sec, ms; char dot1, dot2; std::stringstream ss(timeStr);
	if (ss >> min >> dot1 >> sec >> dot2 >> ms) {
...
void loadChart(const std::string& filename) {
	...
	if (std::getline(file, line)) { 
		...
		std::string timeStr; 
		ss >> key >> timeStr; 
		chartTotalDuration = parseTime(timeStr);
		if (chartTotalDuration < 5000) chartTotalDuration = 60000; // 保护
	} else {
		chartTotalDuration = 60000;
	}
...
// 统一时间基准：使用 GetTickCount64()，返回从 gameStartTime（点击时刻）起的毫秒数
long long getCurrentTimeMs() {
	// 倒计时阶段 realTime 从 -2000ms 开始，倒计时结束归零
	return GetTickCount64() - gameStartTime - COUNTDOWN_DURATION - settings.audioOffset;
}
...
void presentConsole() {
	...
	for (int i = 0; i < winW * winH; ++i) {
		screenOutputBuffer[i].Char.AsciiChar = ' ';
		screenOutputBuffer[i].Attributes = 0;
	}
	...
	COORD bufSize = { (short)winW, (short)winH };
	COORD bufCoord = { 0, 0 };
...
void renderGame(long long curTime, long long displayTime, bool isBoom, float boomIntensity, bool isRain, bool isFlash) {
	...
	if (settings.enableStarfield) {
		for (int y = 0; y < SCREEN_HEIGHT; ++y) {
			for (int x = currentLaneStartX; x < currentLaneStartX + getLaneWidth() * 4; ++x) {
				if ((x * 17 + (y - displayTime / 50) * 31) % 150 == 0) {
	...
	// 绘制音符（使用 displayTime 计算位置）
	for (auto& note : notes) {
		if (note.processed && !note.isBeingHeld) continue;
		
		int y = currentJudgeLineY - static_cast<int>((note.hitTime - displayTime) * chartSpeed);
		int endY = (note.type == HOLD) ? (currentJudgeLineY - static_cast<int>((note.hitTime + note.duration - displayTime) * chartSpeed)) : y;
		...
		if (endY < SCREEN_HEIGHT && y >= 0) {
			...
			if (note.type == TAP && y >= settings.hiddenLevel && y < currentJudgeLineY + 5) {
				...
				for (int drawY = startDrawY; drawY <= endDrawY; ++drawY) {
					if (drawY == startDrawY || drawY == endDrawY)
						drawString(playfieldLayer, x, drawY, skinCap[settings.noteWidthIndex], colorHold);
					else {
	...
	for (int y = 0; y < SCREEN_HEIGHT; ++y) {
		...
		if (isBoom) {
			float timeFactor = displayTime / 50.0f;
		...
		for (int x = 0; x < SCREEN_WIDTH; ++x) {
			...
			if (srcX >= 0 && srcX < SCREEN_WIDTH) {
				...
				if (glitch) {
					finalLayer[y][x].Char.AsciiChar = "!#$-%^&*"[rand() % 8];
					finalLayer[y][x].Attributes = rand() % 16;
				}
	...
	for (const auto& t : toasts) {
		if (displayTime >= t.startTime && displayTime <= t.endTime) {
	...
	drawString(finalLayer, 2, 7, "TIME: " + std::to_string(displayTime) + " / " + std::to_string(chartTotalDuration) + " ms");
	
	if (countdownActive) {
		long long remaining = COUNTDOWN_DURATION - (GetTickCount64() - countdownStartTime);
		if (remaining > 0) {
			std::string countdownText = "倒计时: " + std::to_string(remaining / 1000 + 1) + "s";
			drawString(finalLayer, SCREEN_WIDTH/2 - 5, 15, countdownText, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY);
		}
	}
	...
	if (isFlash) {
		for (int y = 0; y < SCREEN_HEIGHT; ++y) {
			for (int x = 0; x < SCREEN_WIDTH; ++x) {
				...
				WORD fg = attr & 0x0F, bg = (attr & 0xF0) >> 4;
		...
		for (auto& note : notes) {
			...
			if (note.type == TAP) {
				if (diff <= 0) { 
					...
					laneColors[note.lane] = FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY;
					laneEffectEndTime[note.lane] = curTime + 100;
					hitShakeEndTime = curTime + 80; hitShakeIntensity = 0.3f;
					spawnParticles(note.lane, laneColors[note.lane], 8);
					playTapSound();
					note.processed = true;
				}
			} else if (note.type == HOLD) {
				if (!note.holdStarted) {
					if (diff <= 0) { 
						...
						laneColors[note.lane] = FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY;
						hitShakeEndTime = curTime + 80; hitShakeIntensity = 0.3f;
						spawnParticles(note.lane, laneColors[note.lane], 5);
						playTapSound();
					}
				} else {
					prevKeyState[note.lane] = true;
					laneEffectEndTime[note.lane] = curTime + 100;
					...
					if (curTime >= note.hitTime + note.duration) { 
						...
						laneColors[note.lane] = FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY;
						laneEffectEndTime[note.lane] = curTime + 100;
						hitShakeEndTime = curTime + 80; hitShakeIntensity = 0.4f;
						spawnParticles(note.lane, laneColors[note.lane], 12);
					}
				}
			}
		}
					...
					if (!note.holdStarted) {
						...
						if (isKeyDown) { 
							...
							if (note.isBeingHeld && curTime >= endTime) { 
								...
								laneEffectEndTime[i] = curTime + 100;
								hitShakeEndTime = curTime + 80; hitShakeIntensity = 0.4f;
								spawnParticles(i, laneColors[i], 12);
								break;
							}
							break; 
						} else { 
							if (note.isBeingHeld) {
								if (curTime >= endTime - pT) { 
									...
									judgeDisplayTimer = 30; lastHitTime = curTime;
									laneColors[i] = FOREGROUND_INTENSITY;
									laneEffectEndTime[i] = curTime + 100;
								...
								if (curTime - note.holdReleaseTime > graceLimit) {
									note.processed = true; lastJudge = "MISS"; combo = 0; statMiss++;
									judgeDisplayTimer = 30;
									laneColors[i] = FOREGROUND_INTENSITY;
									laneEffectEndTime[i] = curTime + 100;
									break;
								} else if (curTime >= endTime - pT) {
									note.processed = true; lastJudge = "PERFECT"; combo++; statPerfect++;
									judgeDisplayTimer = 30; lastHitTime = curTime;
									laneColors[i] = FOREGROUND_INTENSITY;
									laneEffectEndTime[i] = curTime + 100;
									spawnParticles(i, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY, 10);
...
void renderResults(long long uiTime) {
	...
	float progress = std::min(1.0f, elapsed / 2000.0f); 
	for(int y=0; y<SCREEN_HEIGHT; ++y) {
		if(rand()%10 == 0) for(int x=0; x<SCREEN_WIDTH; ++x) if(rand()%30 == 0) {
			finalLayer[y][x].Char.AsciiChar = rand()%94 + 33;
	...
	if (elapsed > 2000) {
		WORD rColor = FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED;
		...
		drawString(finalLayer, 10, 12, r1, rColor);
		drawString(finalLayer, 10, 13, r2, rColor);
		drawString(finalLayer, 10, 14, r3, rColor);
		drawString(finalLayer, 10, 15, r4, rColor);
		drawString(finalLayer, 10, 16, r5, rColor);
...
void renderMenu(long long uiTime) {
	...
	if (stars.empty()) {
		for (int i = 0; i < 80; ++i) stars.push_back({rand()%SCREEN_WIDTH, rand()%SCREEN_HEIGHT});
	}
	for (auto& s : stars) {
		finalLayer[s.second][s.first].Char.AsciiChar = '.';
		finalLayer[s.second][s.first].Attributes = FOREGROUND_GREEN;
		if (rand()%100 < 2) { s.first = (s.first + 1) % SCREEN_WIDTH; s.second = (s.second + 1) % SCREEN_HEIGHT; }
	}
	...
	for (int i = 0; i < MENU_ITEM_COUNT; ++i) {
		int x = SCREEN_WIDTH / 2 - 8, y = 18 + i * 2;
		int slideOffset = std::max(0, 30 - (int)(elapsed / 15) + i * 5);
		x += slideOffset;
		if (x >= SCREEN_WIDTH) continue;
		if (i == selectedMenuItem) {
			int wave = (sin(uiTime / 100.0) + 1.0) * 2;
			drawString(finalLayer, x - 4 - wave, y, std::string(wave, ' ') + "-> " + menuItems[i], FOREGROUND_GREEN | FOREGROUND_INTENSITY);
	...
	if (elapsed > 1000) {
		...
		drawString(finalLayer, 27, 35, "(C) 2026 Demo 0.23", FOREGROUND_BLUE | FOREGROUND_INTENSITY);
	}
	presentConsole();
}
...
void renderSettings(long long uiTime) {
	...
	for (int i = 0; i < 9; ++i) {
		...
		if (i == selectedSettingItem) {
			...
			std::string brkLeft = pulse ? "[ " : "  ", brkRight = pulse ? " ]" : "  ";
...
void processChartSelectInput() {
	...
	if (!chartFiles.empty()) {
		...
		if (currentEnter && !prevEnterState) {
			...
			statPerfect = statGreat = statBad = statMiss = 0;
			hitShakeEndTime = 0;
			particles.clear();
			currentLaneStartX = getBaseLaneStartX();
			
			// ---- 关键改动：音符时间偏移 + 延迟播放音乐 ----
			// 1. 将所有音符的 hitTime 增加 COUNTDOWN_DURATION（2秒）
			
			// 2. 记录点击时刻作为 gameStartTime
			gameStartTime = GetTickCount64();
			countdownStartTime = gameStartTime;
			countdownActive = true;
			gameStarted = false;
			musicStarted = false;
			
			// 3. 仅打开音频设备，暂不播放
			sendAudioCommand("open \"music/" + musicFileName + "\" type mpegvideo alias bgm");
			// 不执行 play bgm
...
std::string recMusicName, recChartName;
long long recDurationMs = 0;

void setupRecorder() {
	SetConsoleActiveScreenBuffer(GetStdHandle(STD_OUTPUT_HANDLE));
	system("cls");
	std::cout << "===== 谱面录制模式 =====\n请输入同目录下音乐名字 (如: music.mp3): ";
	std::cin >> recMusicName;
	std::cout << "请输入歌曲时长 (毫秒，例如: 120000 代表2分钟): ";
	std::cin >> recDurationMs;
	std::cout << "请输入新谱面名字 (如: mychart.txt): ";
	std::cin >> recChartName;
...
void processRecording() {
	...
	sendAudioCommand("open \"music/" + recMusicName + "\" type mpegvideo alias bgm");
	sendAudioCommand("play bgm");
	while (true) {
	...
	sendAudioCommand("stop bgm");
	sendAudioCommand("close bgm");
	outfile.close();
	SetConsoleActiveScreenBuffer(GetStdHandle(STD_OUTPUT_HANDLE));
	system("cls");
...
int main() {
	...
	if (!initSystemDLLs()) {
		std::cout << "Warning: Failed to load winmm.dll for Audio & Clock." << std::endl;
	...
	while (currentState != STATE_EXIT) {
		...
		if (currentState == STATE_MENU) { processMenuInput(); renderMenu(uiTime); targetSleep = 16; }
			...
			// 倒计时结束 → 播放音乐并启用判定
			if (countdownActive && !musicStarted) {
				if (GetTickCount64() - countdownStartTime >= COUNTDOWN_DURATION) {
					countdownActive = false;
					gameStarted = true;
					musicStarted = true;
					sendAudioCommand("play bgm");   // 此时才播放音乐
				}
			}
			
			long long realTime = getCurrentTimeMs();   // 从点击时刻起算
			long long displayTime = realTime;          // 音符位置直接用 realTime，因为 hitTime 已偏移 2s
			
			if (gameStarted && realTime > chartTotalDuration + 2000) {
				sendAudioCommand("stop bgm");
				sendAudioCommand("close bgm");
			...
			for (const auto& a : anomalies) {
				if (displayTime >= a.startTime) {
					if (a.type == X_SHIFT) gOffX += (displayTime >= a.endTime) ? a.val : a.val * ((float)(displayTime - a.startTime) / (a.endTime - a.startTime));
					else if (a.type == Y_SHIFT) gOffY += (displayTime >= a.endTime) ? a.val : a.val * ((float)(displayTime - a.startTime) / (a.endTime - a.startTime));
				}
				if (displayTime >= a.startTime && displayTime <= a.endTime) {
					if (a.type == BOOM) { isBoom = true; boomIntensity = 1.0f - (float)(a.endTime - displayTime) / (a.endTime - a.startTime); } 
					else if (a.type == RAIN) { isRain = true; } 
					else if (a.type == FLASH) { isFlash = true; }
				}
			}
			
			currentLaneStartX = getBaseLaneStartX() + (int)gOffX;
			currentJudgeLineY = BASE_JUDGE_LINE_Y + (int)gOffY;
			
			if (currentState == STATE_PLAYING) {
				processInput(realTime);
				renderGame(realTime, displayTime, isBoom, boomIntensity, isRain, isFlash);
			}
